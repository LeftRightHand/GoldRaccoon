...
@property (nonatomic, strong) void (^completed)(GRRequest *request);
@property (nonatomic, strong) void (^listDirectoryCompleted)(GRRequest *request, NSArray *listing);
@property (nonatomic, strong) void (^failed)(GRRequest *request, NSError *error);
@property (nonatomic, strong) void (^percent)(float percent);
...
- (id<GRRequestProtocol>)addRequestForListDirectoryAtPath:(NSString *)path sucess:(void (^)(GRRequest *, NSArray *))sucess failed:(void (^)(GRRequest *, NSError *))failed {
    _listDirectoryCompleted = sucess;
    _failed = failed;
    return [self _addRequestOfType:[GRListingRequest class] withPath:path];
}
...
- (id<GRRequestProtocol>)addRequestForCreateDirectoryAtPath:(NSString *)path sucess:(void (^)(GRRequest *))sucess failed:(void (^)(GRRequest *, NSError *))failed {
    _completed = sucess;
    _failed = failed;
    return [self _addRequestOfType:[GRCreateDirectoryRequest class] withPath:path];
}
...
- (id<GRDataExchangeRequestProtocol>)addRequestForDownloadFileAtRemotePath:(NSString *)remotePath toLocalPath:(NSString *)localPath progress:(void (^)(float))progress sucess:(void (^)(GRRequest *))sucess failed:(void (^)(GRRequest *, NSError *))failed
{
    _percent = progress;
    _completed = sucess;
    _failed = failed;
...
- (id<GRDataExchangeRequestProtocol>)addRequestForUploadFileAtData:(NSData *)data toRemotePath:(NSString *)remotePath progress:(void (^)(float))progress sucess:(void (^)(GRRequest *))sucess failed:(void (^)(GRRequest *, NSError *))failed {
    _percent = progress;
    _completed = sucess;
    _failed = failed;
    return [self _addDataExchangeRequestOfType:[GRUploadRequest class] withData:data remotePath:remotePath];
}

- (id<GRDataExchangeRequestProtocol>)addRequestForUploadFileAtLocalPath:(NSString *)localPath toRemotePath:(NSString *)remotePath progress:(void (^)(float percent))progress sucess:(void (^)(GRRequest *request))sucess failed:(void (^)(GRRequest *request, NSError *error))failed
{
    _percent = progress;
    _completed = sucess;
    _failed = failed;
...
- (void)requestCompleted:(GRRequest *)request
{
    ...
    if ([request isKindOfClass:[GRListingRequest class]]) {
        ...
        _listDirectoryCompleted ? _listDirectoryCompleted(request, listing) : nil;
    ...
    if ([request isKindOfClass:[GRCreateDirectoryRequest class]]) {
        _completed ? _completed(request) : nil;
    ...
    if ([request isKindOfClass:[GRUploadRequest class]]) {
        _completed ? _completed((GRUploadRequest *)request) : nil;
    ...
    // download request
        ...
        if (writeToFileSucceeded && !writeError) {
            ...
            _failed ? _failed(request, writeError) : nil;
...
{
//    GRCreateDirectoryRequest *dirRequest = request;
//    for (int i = 0; i < dirRequest.listrequest.filesInfo.count; i++) {
//        NSDictionary *dict = dirRequest.listrequest.filesInfo[i];
//        NSString *name = [dict objectForKey:kCFFTPResourceName];
//        NSLog(@"%@", name);
//    }
    NSError *error = [NSError errorWithDomain:@"com.albertodebortoli.goldraccoon" code:-1000 userInfo:@{@"message": request.error.message}];
    _failed ? _failed(request, error) : nil;
...
- (id<GRDataExchangeRequestProtocol>)_addDataExchangeRequestOfType:(Class)clazz withData:(NSData *)data remotePath:(NSString *)remotePath
{
    id<GRDataExchangeRequestProtocol> request = [[clazz alloc] initWithDelegate:self datasource:self];
    request.path = remotePath;
